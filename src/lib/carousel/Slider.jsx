...
import CircularArray from './itemsArray';
...
class Slider extends Component {
  constructor(props) {
    ...
    ['', 'Webkit', 'Moz', 'O', 'ms'].every((prefix) => {
      ...
      if (
        ...
        typeof document.body.style[e] !== 'undefined'
      ) {
        this.xform = e;
        return false;
      }
    ...
    this.handleResize = throttle(handleResize.bind(this), 1000, {
      leading: true,
    });
    this.handleResizeHeight = throttle(
      handleResizeHeight.bind(this),
      500
    );
  ...
  shouldComponentUpdate(nextProps, nextState) {
    ...
    const {
      settings: { slidesPerRow: originPerRow, rows: originRows },
    } = this.state;
  ...
  init = () => {
    ...
    for (
      ...
      for (
        ...
        newSlide.push(
          <div className="carousel-row" key={10 * i + j}>
            {row}
          </div>
        );
      }
    ...
    if (
      !isEqual(get(this.state, 'settings'), settings) &&
      this.mounted
    ) {
   ...
   */
  setRef = (element) =>
    this.setState({ SliderRef: element }, () => {
      ...
      if (virtualList) {
        ...
        this.forceUpdate(() => {
          this.items = new CircularArray(
            element.querySelectorAll('.carousel-item')
          );
          this.virtualItem = null;
        });
      } else {
  ...
  autoPlay = () => {
    const {
      SliderRef,
      settings: { autoplay, autoplaySpeed, pauseOnHover },
    } = this.state;
  ...
  connectObserver = () => {
    if (!this.resizeObserver) {
      ...
      this.resizeObserver.observe(
        SliderRef.querySelector('.carousel-item')
      );
    } else {
  ...
  autoScroll = (type) => {
    ...
    if (this.amplitude) {
      ...
      const delta =
        this.amplitude * Math.exp(-elapsed / settings.duration);
   ...
   */
  scroll = (type, x) => {
    const { SliderRef, width, settings, activeIndex } = this.state;
    const {
      centerMode,
      beforeChange,
      afterChange,
    ...
    if (centerMode) {
      if (slidesToShow % 2 === 0) {
        ...
        alignment = `translateX(${
          (SliderRef.clientWidth - width) / 2 - settings.centerPadding
        }px)`;
      }
    } else {
    ...
    if (
      ...
      switch (scrollType) {
        ...
        case 'dots': {
          newIndex =
            this.scrollOptions.index * this.scrollOptions.dotsScroll;
          break;
        }
        case 'autoplay': {
    ...
    this.virtualItem =
      this.virtualItem ||
      new CircularArray(
        SliderRef.querySelectorAll('.carousel-item'),
        this.items
      );
    ...
    if (scrollItem.length <= slidesToShow) {
      ...
      if (el) {
        ...
        if (el.classList.contains('active')) {
          each(SliderRef.querySelectorAll('.carousel-item'), (ele) =>
      ...
      !this.noWrap ||
      (this.center >= 0 && this.center < scrollItem.length)
    ) {
      ...
      if (el) {
        ...
        const transformString = `${alignment} translateX(${
          -delta / 2
        }px) translateX(${dir * settings.shift * tween * i}px)`;
        this.updateItemStyle(el, transformString);
      }
    }
    if (centerMode) {
      ...
      for (i = 1; i <= half; i += 1) {
        ...
        if (!this.noWrap || this.center + i < scrollItem.length) {
          ...
          if (el) {
            const transformString = `${alignment} translateX(${
              settings.shift + (this.dim * i - delta) / 2
            }px)`;
            this.updateItemStyle(el, transformString);
          }
        }
      ...
      for (i = 0; i < half; i += 1) {
        ...
        if (
          !this.noWrap ||
          this.center + slidesToShow + i < scrollItem.length
        ) {
          el = this.getItem(
            scrollItem,
            this.wrap(this.center + slidesToShow + i)
          );
          if (el) {
            const transformString = `${alignment} translateX(${
              settings.shift +
              (this.dim * (slidesToShow + i) - delta) / 2
            }px)`;
            this.updateItemStyle(el, transformString);
          }
        }
        ...
        if (
          ...
          el = this.getItem(
            scrollItem,
            this.wrap(this.center - i - 1)
          );
          if (el) {
            const transformString = `${alignment} translateX(${
              -settings.shift + (-this.dim * (i + 1) - delta) / 2
            }px)`;
            this.updateItemStyle(el, transformString);
          }
        }
    ...
    // Don't show wrapped items.
    if (
      (!this.noWrap || this.center < this.items.length) &&
      ...
      if (el) {
        if (!el.classList.contains('active')) {
          ...
          if (this.beforeChangeTrigger && this.mounted) {
        ...
        const transformString = `${alignment} translateX(${
          -delta / 2
        }px) translateX(${dir * settings.shift * tween}px)`;
        this.updateItemStyle(el, transformString);
      }
    }
    this.adaptHeight();
  ...
  widthInit = () => {
    ...
    const { centerMode, centerPadding, slidesToShow } = settings;
    if (SliderRef) {
      let padding = 0;
      if (typeof centerPadding === 'string') {
        ...
        padding = 50;
        console.warn(
          'centerPadding have to be number or string like 50px'
        );
      }
  ...
  createVirtualList = () => {
    const {
      settings: { slidesToShow, overScan },
      activeIndex,
    } = this.state;
      ...
      if (this.endIndex >= 0 && typeof this.endIndex === 'number') {
        ...
        if (
          activeIndex + this.endIndex <
            this.newChildren.length + this.scrollDistance &&
          activeIndex + this.endIndex >=
            this.newChildren.length - this.scrollDistance &&
          (activeIndex >=
            this.newChildren.length - this.scrollDistance ||
          ...
          if (this.endIndex + activeIndex < this.newChildren.length) {
            if (this.endIndex < activeIndex) {
              buffer =
                this.newChildren.length - activeIndex + this.endIndex;
            } else {
              buffer =
                this.newChildren.length - this.endIndex + activeIndex;
            }
        ...
        for (let j = 0; j < buffer; j += 1) {
          const rightIndex = this.items.getIndex(
            newActiveIndex + i + j
          );
          const leftIndex = this.items.getIndex(
            newActiveIndex - i - j
          );
          switch (type) {
            case 'arrows': {
  ...
  render() {
        ...
        Object.assign(dotProps, {
          ...
          clickHandler: (options) => {
            ...
            if (activeIndex > options.index) {
              right =
                this.newChildren.length - activeIndex + options.index;
              left = activeIndex - options.index;
              direction = right < left ? 'right' : 'left';
            } else {
              right = options.index - activeIndex;
              left =
                this.newChildren.length - options.index + activeIndex;
              direction = right <= left ? 'right' : 'left';
            }
            this.scrollType = {
              type: 'dots',
    ...
    const judge = this.items
      ? settings.slidesToShow < this.items.length
      : false;
    /*  Slide  */
    let component = (
      <Fragment>
